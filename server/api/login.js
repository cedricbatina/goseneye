...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.log("Tentative de connexion réussie avec l'email :", email);

    // Renvoyer un message de succès avec les informations de l'utilisateur et le token JWT
    return {
      message: "Connexion réussie",
      token, // Envoyer le token pour que le client le stocke dans localStorage
      user: {
        id: user[0].id,
        email: user[0].email,
        username: user[0].username, // Ajoute le username ici
    ...
    console.error("Erreur lors de la connexion :", error.message); // Maintenant, c'est dans le bon contexte
    return { error: "Erreur lors de la connexion" };
  }
});
